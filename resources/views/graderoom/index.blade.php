                                    ...
                                    <template x-for="grade in grades" :key="grade.id">
                                        <tr class="text-center border-b hover:bg-gray-50 text-sm" x-data="{ edit: false }">
                                            ...
                                            <td class="px-3 py-2 text-sm flex mx-auto whitespace-nowrap items-center justify-center gap-1" x-show="!edit">
                                                <div @click="editGrade(grade.id)" class="px-2 py-1 bg-white border text-sm flex my-auto items-center gap-1 cursor-pointer hover:border-gray-400 transition">
                                                ...
                                                <div @click="deleteGrade(grade.id)" class="px-2 py-1 bg-white border text-sm flex my-auto items-center gap-1 cursor-pointer hover:border-gray-400 transition">
                                                    <a class="flex my-auto items-center gap-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
            ...
            <div x-data="RoomDataTable()" x-init="fetchRooms" class="flex flex-col w-full space-y-2">
                ...
                <div class="bg-white flex flex-col border rounded shadow-sm mt-2 overflow-y-auto">
                    ...
                    <div class="px-5 py-5 overflow-y-auto">
                        <div class="flex gap-2 justify-between">
                            {{-- <p class="text-sm my-auto">Search</p> --}}
                            <x-text-input x-model="search" @input.debounce.500="fetchRooms()" class="py-1 text-sm" type="text" placeholder="Search" name="search" :value="old('search')" />
                            <div class="flex gap-2">
                                <p class="text-sm my-auto">Show</p>
                                <select x-model="show" @change="fetchRooms()" class="border-gray-300 text-sm py-1 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    {{-- <option value="0" selected disabled>Show</option> --}}
                                    <option value="5" selected>5</option>
                                    <option value="10">10</option>
                                    <option value="20">20</option>
                            ...
                            <table class="table-auto w-full shadow-sm overflow-y-auto">
                                ...
                                <tbody>
                                    <template x-for="room in rooms" :key="room.id">
                                        <tr class="text-center border-b hover:bg-gray-50 text-sm" x-data="{ edit: false }">
                                            <td class="px-3 py-2 text-sm" x-show="!edit" x-text="room.name"></td>
                                            <td class="px-3 py-2 text-sm" x-show="!edit" x-text="room.code"></td>
                                            <td class="px-3 capitalize py-2 text-sm" x-show="!edit" x-text="room.status"></td>
                                            <td class="px-3 py-2 text-sm flex mx-auto whitespace-nowrap items-center justify-center gap-1" x-show="!edit">
                                                <div @click="editRoom(room.id)" class="px-2 py-1 bg-white border text-sm flex my-auto items-center gap-1 cursor-pointer hover:border-gray-400 transition">
                                                    <a class="flex my-auto items-center gap-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                                        </svg>                                              
                                                ...
                                                <div @click="deleteRoom(room.id)" class="px-2 py-1 bg-white border text-sm flex my-auto items-center gap-1 cursor-pointer hover:border-gray-400 transition">
                                                    <a class="flex my-auto items-center gap-1">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                                                        </svg>                                              
                                    ...
                                    <tr x-show="rooms.length === 0" class="text-center border-b hover:bg-gray-50 text-sm">
                        ...
                        <div class="flex justify-between w-full mt-2">
                            <div class="text-sm my-auto">
                                <p>
                                    Show <span x-text="rooms.length"></span> of <span x-text="total"></span> entries
                                </p>
                            </div>
                            <div class="flex rounded border my-auto">
                                <p @click="doublePreviousPage()" class="px-1 py-1 border-l hover:cursor-pointer hover:bg-gray-50 text-gray-500">
...
<script>
    function RoomDataTable(){
        return {
            ...
            message: null,
            fetchRooms(){
                fetch('/rooms/getrooms?search=' + this.search + '&show=' + this.show + '&page=' + this.page)
                .then(res => res.json())
                .then(data => {
            ...
            deleteRoom(id){
                if(confirm('Are you sure you want to delete this room level?')){
    ...
    function GradeDataTable(){
        return {
            ...
            current_page: 0,
            editGrade(id){
                window.location.href = `/grades/edit/${id}`
            },
            fetchGrades(){
